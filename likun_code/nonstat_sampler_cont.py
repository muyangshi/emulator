...
if __name__ == "__main__":
   ...
   rank = comm.Get_rank()
   size = comm.Get_size()
   thinning = 10; echo_interval = 50; n_updates = 80001
   ...
   if rank==0:
       with open(filename, 'rb') as f:
           ...
           if(Rt_knots_trace.shape[0]<n_updates_thinned):
               ...
               phi_knots_trace = np.pad(phi_knots_trace, ((0, add_length),(0,0)),'constant', constant_values=np.nan)
   ...
   R_weights =  initial_values['R_weights'] # Updated in each rank
   Stations =  initial_values['Stations']
   phi_vec = initial_values['phi_vec'] # Updated in each rank
   phi_at_knots = initial_values['phi_at_knots'] # only needed in rank 0
   radius_from_knots = initial_values['radius_from_knots']  # only needed in rank 0
   gamma = initial_values['gamma'] # saved in each rank
   gamma_vec = initial_values['gamma_vec'] # Updated in each rank
   range_vec = initial_values['range_vec'] # NEEDED
   range_at_knots = initial_values['range_at_knots'] # only needed in rank 0
   nu = initial_values['nu'] # saved in each rank
   X = initial_values['X'] # Deprecated
   Rt_at_knots = initial_values['Rt_at_knots'] # Updated in each rank
   R_s = initial_values['R_s'] # Deprecated
   Design_mat = initial_values['Design_mat']
   beta_gev_params = initial_values['beta_gev_params'] # only needed in rank 0 but needed to broadcast at first
   Time = initial_values['Time']
   
   # Propagate the GEV parameter matrices
   ...
   # Two rank-dependent vectors
   Xt = utils.gev_2_RW(Y[:,rank], phi_vec, gamma_vec, Loc[:,rank], Scale[:,rank], Shape[:,rank])
   Rt_s = R_weights @ Rt_at_knots
   ...
   one_vec = np.ones(n_s)
   range_at_knots = comm.bcast(range_at_knots,root=0)
   range_vec = phi_range_weights @ range_at_knots
   ...
   for iter in np.arange(iter_current+1,n_updates):
       ...
       if (iter % thinning) == 0:
           ...
           if rank == 0:
               ...
               sigma_m['phi'] = np.exp(np.log(sigma_m['phi']) + gamma2*(phi_accept/thinning - r_opt_2d))
               phi_accept = 0
               prop_Sigma['phi'] = prop_Sigma['phi'] + gamma1*(np.cov(phi_knots_within_thinning) - prop_Sigma['phi'])
               check_chol_cont = True
               while check_chol_cont:
                   try:
                       # Initialize prop_C
                       np.linalg.cholesky(prop_Sigma['phi'])
                       check_chol_cont = False
                   except  np.linalg.LinAlgError:
                       prop_Sigma['phi'] = prop_Sigma['phi'] + eps*np.eye(n_phi_range_knots)
                       print("Oops. Proposal covariance matrix is now:\n")
                       print(prop_Sigma['phi'])
       ...
       if (iter / thinning) % echo_interval == 0:    
           ...
           if rank == 0:
               ...
               fig = plt.figure(figsize = (8.75, 11.75))
               plt.subplot2grid(grid_size, (0,0)) 
               plt.plot(phi_knots_trace[:,0], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[0]')
               plt.subplot2grid(grid_size, (0,1))
               plt.plot(phi_knots_trace[:,1], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[1]')
               plt.subplot2grid(grid_size, (1,0))  
               plt.plot(phi_knots_trace[:,2], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[2]')
               plt.subplot2grid(grid_size, (1,1)) 
               plt.plot(phi_knots_trace[:,3], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[3]')
               plt.subplot2grid(grid_size, (2,0))  
               plt.plot(phi_knots_trace[:,4], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[4]')
               plt.subplot2grid(grid_size, (2,1))  
               plt.plot(phi_knots_trace[:,5], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[5]')
               plt.subplot2grid(grid_size, (3,0))  
               plt.plot(phi_knots_trace[:,6], color='gray', linestyle='solid')
               plt.ylabel(r'$phi_{knots}$[6]')
               plt.subplot2grid(grid_size, (3,1))  
               plt.plot(phi_knots_trace[:,7], color='gray', linestyle='solid')
